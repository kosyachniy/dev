...
№ | Где | Ошибка | Решение
---|---|---|---
1 | ``` twi/python/auto 3/old.py ``` | Отписка полностью от всех невзаимных |
2 | ``` twi/python/auto 3/new.py ``` | Цикл проходит по последним пользователям повторно (уже обработанным) |
3 | ``` twi/python/auth.php ``` | Авторизация не работает автоматически |
4 | ``` twi/python/auto 3/user.py ``` | Копирует посты у тех, на кого подписался |
...
Указатели / переменные | указатели | переменные |  |  |
